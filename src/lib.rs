...
//! - **Negation**: `not` — the rest of the schema generates candidates, and
//!   ones the negated sub-schema accepts are redrawn, up to a bounded number of
//!   attempts; conjoined negations forbid the union of what each forbids. The
//!   satisfaction check behind this is partial, and a candidate it cannot
//!   decide is accepted: `not` against `enum`, `const`, `type`, `required`,
//!   `properties`, the length/count/numeric bounds and nested
//!   `allOf`/`anyOf`/`oneOf`/`$ref` is enforced exactly, while a `not` over
//!   `pattern`, `format`, or any other keyword is best-effort
...
mod tests {
    ...
    /// The `73-display_name` corpus shape: a free-form string with a couple of
    /// placeholder values ruled out.
    #[test]
    fn test_not_enum_is_never_generated() {
        let schema = json!({
            "type": "string",
            "minLength": 1,
            "maxLength": 255,
            "not": {"enum": ["unknown", "untitled"]}
        });
        let mut rng = seeded_rng();
        for _ in 0..500 {
            let value = generate(&schema, &mut rng).expect("generation should succeed");
            assert!(
                value != json!("unknown") && value != json!("untitled"),
                "generated a value the schema forbids: {value}"
            );
        }
        generate_and_validate_n(&schema, 200);
    }

    /// The negated `const` is reachable — a one-of-three enum makes the
    /// rejection do visible work rather than pass by improbability.
    #[test]
    fn test_not_const_within_enum() {
        let schema = json!({"enum": ["a", "b", "c"], "not": {"const": "b"}});
        let mut rng = seeded_rng();
        let mut seen = std::collections::BTreeSet::new();
        for _ in 0..200 {
            let value = generate(&schema, &mut rng).expect("generation should succeed");
            assert_ne!(value, json!("b"), "generated the forbidden variant");
            seen.insert(value.to_string());
        }
        assert_eq!(
            seen.len(),
            2,
            "expected both allowed variants, got {seen:?}"
        );
    }

    #[test]
    fn test_not_type_narrows_type_union() {
        let schema = json!({"type": ["string", "null"], "not": {"type": "null"}});
        let mut rng = seeded_rng();
        for _ in 0..200 {
            let value = generate(&schema, &mut rng).expect("generation should succeed");
    ...
    #[test]
    fn test_not_nested_in_property() {
        let schema = json!({
            "type": "object",
            "properties": {
                "status": {"enum": ["draft", "live"], "not": {"const": "draft"}}
            },
            "required": ["status"]
        });
        let mut rng = seeded_rng();
        for _ in 0..100 {
            let value = generate(&schema, &mut rng).expect("generation should succeed");
            assert_eq!(value.get("status"), Some(&json!("live")));
        }
    }

    /// `not` wrapping a composition keyword still filters, because the rest of
    /// the schema — branch pick included — generates the candidate first.
    #[test]
    fn test_not_alongside_one_of() {
        let schema = json!({
    ...
    /// A `not` that rules out one branch does not doom the schema: each
    /// attempt regenerates the rest from scratch, branch pick included.
    #[test]
    fn test_not_ruling_out_one_any_of_branch() {
        let schema = json!({
    ...
    #[test]
    fn test_not_merged_from_all_of_members() {
        let schema = json!({
            "enum": ["a", "b", "c"],
    ...
    #[test]
    fn test_not_within_contains() {
        let schema = json!({
            "type": "array",
            "items": {"enum": ["a", "b"]},
            "minItems": 2,
            "contains": {"enum": ["a", "b"], "not": {"const": "a"}}
        });
        let mut rng = seeded_rng();
        for _ in 0..100 {
            let value = generate(&schema, &mut rng).expect("generation should succeed");
            let items = value.as_array().expect("should generate an array");
            assert!(items.contains(&json!("b")), "expected a \"b\" in {value}");
        }
    }

    #[test]
    fn test_not_unsatisfiable_returns_error() {
        let schema = json!({"type": "string", "not": {"type": "string"}});
        let mut rng = seeded_rng();
        let err = generate(&schema, &mut rng).unwrap_err();
        assert!(
            matches!(err, crate::Error::ConflictingConstraints { .. }),
    ...
    #[test]
    fn test_not_true_returns_error() {
        let schema = json!({"type": "string", "not": true});
        let mut rng = seeded_rng();
        let err = generate(&schema, &mut rng).unwrap_err();
        assert!(
            matches!(err, crate::Error::ConflictingConstraints { .. }),
    ...
    #[test]
    fn test_not_false_constrains_nothing() {
        let schema = json!({"type": "integer", "minimum": 1, "maximum": 3, "not": false});
        generate_and_validate_n(&schema, 50);
    }

    /// A `not` the satisfaction checker cannot decide — `pattern` needs a
    /// regex engine — is accepted rather than enforced, the same posture as an
    /// unsupported `pattern` during generation.
    #[test]
    fn test_not_undecidable_sub_schema_is_accepted() {
        let schema = json!({"type": "string", "not": {"pattern": "^[a-z]+$"}});
        let mut rng = seeded_rng();
        for _ in 0..20 {
            generate(&schema, &mut rng).expect("generation should succeed");
        }
    }

    /// `not` over a `$ref` resolves the target before checking it.
    #[test]
    fn test_not_ref_is_resolved() {
        let schema = json!({
            "$defs": {"placeholder": {"const": "tbd"}},
            "enum": ["tbd", "ready"],
